...
Everything lives on one Administration page - **Administration → Message of the day**. FREE stores one plain-text message with an active start and end, displayed in a blue bar with initMAX branding. PRO adds unlimited stored messages, early announcements with "Show since", custom colours, repetition, HTML and per-user close memory; its bar has no initMAX branding. Paid controls are greyed with a padlock in FREE. After a downgrade, existing records remain available for editing and deletion, but only the oldest record can be displayed and additional messages cannot be created.
...
| Set the active start and end of a message | ✅ | ✅ |
...
| **Show since - announce a message before its active window** | ❌ | ✅ |
| **Custom message bar colours** | ❌ | ✅ |
| **Unlimited stored messages (FREE: one)** | ❌ | ✅ |
| **Message bar without initMAX branding** | ❌ | ✅ |
